...
df2 = pd.read_csv('./mycsvfile.csv')
author = df2['author_name']
return_2021 = df2['return_2021']
first_author = df2['first_author']
last_author = df2['last_author']
gender2 = df2['gender']
...
def firstTime():
    names = ['Male First Time', 'Female First Time']
    mean_values=[0,0]
    ...
    for index, value in enumerate(author):
        ...
        if first_time and gender2[index]=='f': 
            mean_values[1]+=1
        elif first_time and gender2[index]=='m': 
            mean_values[0]+=1
...
def avgPublications(): 
    ...
    entire_array = fem_pub_array + male_pub_array
    entire_array.sort()
    print(len(entire_array)*.9)
    print(entire_array[int(len(entire_array)*.9)])

    ten_fem_first_count = 0
    ten_fem_first_publications = 0
    ten_fem_pub_array = []
    ten_male_first_count = 0
    ten_male_first_publications = 0
    ten_male_pub_array=[]

    for idx, val in enumerate(gender):
        gender_list = val.split(';')
        if gender_list[0]=='f':
            if int(authors_publications[idx].split(';')[-1].replace(',',''))!=-1 and int(authors_publications[idx].split(';')[-1].replace(',',''))<=235:
                ten_fem_first_count+=1
                ten_fem_first_publications+=int(authors_publications[idx].split(';')[-1].replace(',',''))
                ten_fem_pub_array.append(int(authors_publications[idx].split(';')[-1].replace(',','')))
        elif gender_list[0]=='m':
            if int(authors_publications[idx].split(';')[-1].replace(',',''))!=-1 and int(authors_publications[idx].split(';')[-1].replace(',',''))<=235:
                ten_male_first_count+=1
                ten_male_first_publications+=int(authors_publications[idx].split(';')[-1].replace(',',''))
                ten_male_pub_array.append(int(authors_publications[idx].split(';')[-1].replace(',','')))


    print('avg num of publications for last authors with first fem authors:', ten_fem_first_publications/ten_fem_first_count)
    print('avg num of publications for last authors with first male authors:', ten_male_first_publications/ten_male_first_count)
    print('sd fem:', st.stdev(ten_fem_pub_array))
    print('sd fem:', st.stdev(ten_male_pub_array))
